...
exports.getAll = async (req, res) => {
  try {
    const {
      city, propertyType, propertyAdType,
      minPrice, maxPrice, bedrooms,
      minArea, maxArea, isRecommended,
      page = 1, limit = 9,
    } = req.query;
        ...
        { $match: match },
        {
          $addFields: {
            statusOrder: {
              $switch: {
                branches: [
                  { case: { $and: [{ $eq: ['$isRecommended', true] }, { $eq: ['$status', 'available'] }] }, then: 0 },
...
exports.create = async (req, res) => {
  try {
    ...
    const {
      title, description, propertyType, propertyAdType,
      price, city, fullAddress, gmapsLink,
      amenities, bedrooms, bathrooms,
      buildingArea, surfaceArea, floor,
    } = req.body;

    // Premium landlords never get pending_payment — unlimited posts
    // Free landlords: first post is free (available), subsequent need listing fee
    let status = 'available';
    if (!landlord.freePostUsed) {
      status = 'available'; // first post free
    } else if (landlord.subscriptionStatus !== 'premium') {
      status = 'pending_payment'; // free tier, used free post → needs listing fee
    }
    ...
    res.status(201).json({ property, requiresPayment: status === 'pending_payment' });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    if (req.body.amenities) {
      updates.amenities = Array.isArray(req.body.amenities) ? req.body.amenities : JSON.parse(req.body.amenities);
    }
...
exports.toggleRecommended = async (req, res) => {
  try {
    ...
    if (!property.isRecommended && landlord.recommendedPinsLeft <= 0) {
      return res.status(400).json({ message: 'No recommended pins left. Upgrade or wait for renewal.' });
    }

    const newRecommended = !property.isRecommended;
    const pinChange = newRecommended ? -1 : 1;
    await User.findByIdAndUpdate(req.user._id, { $inc: { recommendedPinsLeft: pinChange } });
